...
const availableRoles=['public','admin','auditor','support'];
export class Users {
  ...
  public async assignRole(id:string, role: string){
    if(!availableRoles.includes(role)){
      throw new Error(`Role ${role} must be one of ${availableRoles.join(', ')}`);
    }
